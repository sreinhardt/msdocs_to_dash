...
def test_docsource_get_contents(root_toc, webserver, html_ok):
    root = deepcopy(root_toc)
    root.items[0].children[0].children[0] = html_ok.encode('utf-8')
    ds = root_toc.parent
    ds.get_contents(WebDriver(), "")
    assert ds._css_files == []
    assert ds._js_files == []
...
def test_docset_get_contents(root_toc, webserver, html_ok, bytes_ok):
...
def test_docsource_write_contents(root_toc, webserver, tmp_path):
    # docsource forces packaging paths
    ds = root_toc.parent.parent
    ds.get_contents(WebDriver(), "")
    ds.write_contents(tmp_path)
...
def test_docsource_make_package(root_toc, webserver, tmp_path):
    ds = root_toc.parent.parent
    ds.get_contents(WebDriver(), "")
    ds.make_database(tmp_path)
    ds.make_package(tmp_path)
    with TarFile.open(f"{tmp_path}/Windows Desktop Api.tar") as tf:
